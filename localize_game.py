...
def extract_tags_for_translation(text) -> tuple[str, dict[str,str]]:
    ...
    # Stack to track opening tags in LIFO order
    # Each entry is (tag_name, placeholder_number, full_tag)
    tag_stack = []
    
    # Track all opening tags seen for auto-closing
    opening_tags_numbers = set()
    
    for match in matches:
        if is_closing_tag(match):
            tag_name = extract_tag_name(match)
            
            # Find the most recent opening tag with this name (LIFO)
            found = False
            placeholder = ""
            for i in range(len(tag_stack) - 1, -1, -1):
                if tag_stack[i][0] == tag_name:
                    # Found matching opening tag
                    number = tag_stack[i][1]
            ...
            tag_stack.append((tag_name, number, match))
            opening_tags_numbers.add(number)
    ...
    # Add closing tags for any unclosed opening tags
    for tag_name, number, full_tag in tag_stack:
...
def update(lang):
    ...
    def translate(key, val):
        ...
        for k,v in tagmap.items():
            translated = translated.replace(k,v)
        print("TRANSLATED: ", lastkey.replace("\n"," "), translated.replace("\n"," "))
        return translated
